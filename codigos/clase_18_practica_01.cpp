...
template <typename T>
Node<T>* CreateTreeFromPreOrder(T pre[], int &preIndex, int low, int high, int &n){
    // Caso Base
    if (preIndex >= n || low > high){
        return NULL;  
    } 
    //El primer nodo en preorden siempre es la raiz, creamos el nodo y aumentamos en uno el indice pre
    Node<T>* root = new Node<T>(pre[preIndex]);
    preIndex = preIndex + 1;  
    // Si solo tenemos un elemento quiere decir que low = high y ya terminamos 
    if (low == high){
        return root;
    }
    //Buscamos el primer elemento mas grande que root(que acabamos de crear)  
    int indice = 0;
    for(indice = low; indice <= high; ++indice) {
        if(pre[indice]>root->data){
            break;  
        }
    }
    // Ahora usamos este indice para dividir pre[] en dos partes.
    root->left  = CreateTreeFromPreOrder(pre, preIndex, low, indice-1, n);  
    root->right = CreateTreeFromPreOrder(pre, preIndex, indice, high, n );
    return root;  
}
...
Node<T>* CreateTreeFromArray(T pre[], int &n){
    ...
    int preIndex = 0;
    return CreateTreeFromPreOrder(pre, preIndex, 0, n-1, n);
}
